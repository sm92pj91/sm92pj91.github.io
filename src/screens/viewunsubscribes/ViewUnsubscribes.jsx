...
class ViewUnsubscribes extends Component {
  state = {
    tdBugs: [],
    redirect: false,
  };
  ...
  componentDidMount() {
    Auth.currentSession({
      ...
      fetch(config.BASE_URL + '/marketing/unsubscribe', {
        method: 'GET',
        headers: {
          ...
          'Authorization': 'Bearer ' + user.getIdToken().getJwtToken()
            ...
            if (res.status !== 200) {
              alert("Error, Unsubscribes not found");
              return;
            }
            res.json()
            .then(response => {
                      ...
                      >
                        {
                          props => (
                              <div>
                                <h4 className="title">Unsubscribed Emails&nbsp;&nbsp;&nbsp;&nbsp;
...
export default ViewUnsubscribes;
